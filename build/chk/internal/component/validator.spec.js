...
describe('Validator', () => {
  describe('validation of "name"', () => {
    ...
    where([{ value: undefined }, { value: 0 }, { value: null }, { value: '' }, { value: 'invalid-name' }, { value: 'invalid_name' }, { value: 'invalid:name' }], () => {
      it('should fail for invalid value', ctx => {
        assert.throws(() => validator.validateName(MyComponent, ctx.value), err => err.message === `invalid component: '${ ctx.value }' must be string with characters only`);
      });
    });
  });
  ...
  describe('validation of "dependencies"', () => {
    ...
    it('should fail for invalid dependency', () => {
      assert.throws(() => validator.validateDependencies(MyComponent, MyType, {
        myDependency: {
          name: 'MyDependency',
          type: {
            typeName: 'Invalid'
